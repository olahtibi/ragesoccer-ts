...
test("Physics advances player position without managing walk animation", function () {
  ...
  fixture.playerHome.velocity.x = world(10);
  fixture.playerHome.velocity.y = 0;

  advancePhysics(fixture, 1, "playersOnly");

  assertNear(fixture.playerHome.position.x, startX + world(10), 0.0001);
...
test("Ball position does not mutate configured initial ball position", function () {
  ...
  fixture.ball.position.x += world(25);
  fixture.ball.position.y += world(30);
...
test("Physics advances every player in the stadium without managing animation", function () {
  ...
  for (var i = 0; i < fixture.stadium.players.length; i++) {
    ...
    fixture.stadium.players[i].velocity.x = world(10);
  ...
  for (var j = 0; j < fixture.stadium.players.length; j++) {
    assertNear(
      fixture.stadium.players[j].position.x,
      startXs[j] + world(10),
      0.0001,
    );
...
test("Cutscene physics advances players and ball without new contacts", function () {
  var fixture = makeFixture();
  fixture.config.physics.maxDeltaSeconds = 1;
  fixture.playerHome.position.x = world(100);
  fixture.playerHome.position.y = world(100);
  fixture.playerHome.velocity.x = world(10);
  fixture.ball.position.x = world(300);
  fixture.ball.position.y = world(300);
  fixture.ball.velocity.x = world(20);

  advancePhysics(fixture, 0.5, "cutscene");

  assertNear(fixture.playerHome.position.x, world(105), 0.0001);
  assertTrue(fixture.ball.position.x > world(300));
...
test("Physics reflects X velocity at a pitch wall", function () {
  ...
  fixture.ball.position.x = fixture.config.pitch.boxTopLeft.x + world(0.5);
  fixture.ball.position.y = fixture.config.pitch.aiCenterY;
  fixture.ball.velocity.x = world(-10);
...
test("Physics reflects Y velocity at a goal wall", function () {
  var fixture = makeFixture();
  fixture.ball.position.x = fixture.config.pitch.goalTopTopLeft.x + world(10);
  fixture.ball.position.y = fixture.config.pitch.goalTopTopLeft.y + world(0.5);
  fixture.ball.velocity.y = world(-10);
...
test("Physics ball-player contact kicks the ball outward", function () {
  ...
  fixture.playerHome.position.x = world(100);
  fixture.playerHome.position.y = world(100);
  fixture.playerHome.velocity.x = world(20);
  fixture.playerHome.velocity.y = 0;
  fixture.ball.position.x = world(104);
  ...
  assertNear(fixture.ball.position.x, world(106.01), 0.0001);
...
test("Disabled out-of-play restarts preserve reflective pitch boundaries", function () {
  var fixture = makeFixture({ outOfPlayRestartsEnabled: false });
  fixture.ball.position.x = fixture.config.pitch.boxTopLeft.x + world(1);
  fixture.ball.position.y = fixture.config.pitch.aiCenterY;
  fixture.ball.velocity.x = world(-100);
...
test("Enabled out-of-play restarts allow the ball to cross pitch boundaries", function () {
  ...
  fixture.ball.position.x = fixture.config.pitch.fieldLeft + world(1);
  fixture.ball.position.y = fixture.config.pitch.aiCenterY;
  fixture.ball.velocity.x = world(-100);
...
test("Physics player-only update advances players without touching the ball", function () {
  ...
  assertEqual(fixture.ball.position.x, world(104));
  assertEqual(fixture.ball.position.y, world(100));
  assertEqual(fixture.ball.velocity.x, world(100));
  assertTrue(fixture.playerHome.position.x > world(100));
});
